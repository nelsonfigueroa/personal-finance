...
48.times do |i|
  ...
  5.times do |x|
    x += 1
    Statement.create!(account_id: x, notes: 'notes', date: date, balance_cents: Faker::Number.between(from: (i * 100_000), to: ((i + 1) * 150_000)), balance_currency: 'USD')
  end
end
...
48.times do |i|
  i += 1
  date = Faker::Date.between(from: (49 - i).months.ago, to: (48 - i).months.ago)
  Transaction.create!(user_id: 1, date: date, category_id: 1, notes: 'Salary', amount_cents: 400_000, amount_currency: 'USD')
end
...
4.times do
  # 12 months
  12.times do
    Transaction.create!(user_id: 1, date: current_date, category_id: 3, notes: 'Rent', amount_cents: 200_000, amount_currency: 'USD')
...
48.times do |i|
  i += 1
  date = Faker::Date.between(from: (49 - i).months.ago, to: (48 - i).months.ago)
  Transaction.create!(user_id: 1, date: date, category_id: 4, notes: 'Groceries', amount_cents: 100_000, amount_currency: 'USD')
end
